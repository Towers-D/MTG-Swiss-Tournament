# MTG Swiss Tournament Tool

## Structure

```markdown
src
+--main.ts
+--App.svelte
|
+--db
|  ---database.ts
|
+--routes
|  +--Home.svelte
|  +--Lobby.svelte
|  +--Round.svelte
|  ---Scores.svelte
|
+--css
| +--main.css
| |  ...
| ---{Other CSS files}
|
---lib
   +--home.ts
   |  ...
   +--{Other page TS files}
   +--utils.ts
   |
   ---tests/
      +--home.test.ts
      |  ...
      -{other test files }
   
```

## Licence

This work is released under a CC-BY-NC-SA 4.0 licence. In regards to this project and the interpretation of the application of this licence we make the following interpretations and assumptions.

1. By Attribution: In order to fulfill the by attribution, we require a link to the repository clearly visible and labeled.
2. Non-Commercial: This project (and all derivations) are not to be used for commercial purposes.
    - This includes using this project to build a paid for website or application locking the tournament tool behind a paywall by any means.
    - This does not prevent the use of the tool at tournaments regardless of entry fee.
3. Share-Alike: These additional interpretations are also attributable to derivations.

## Acknowledgements

Find below a list of libraries and resources used to build this tool.

### Resources Used

### Code Snippets Used

| Function       | Locations  | Desc                                  | Author    | Link                                                                         |
|----------------|------------|---------------------------------------|-----------|------------------------------------------------------------------------------|
| `sanitise`     | `utils.ts` | JavaScript string sanitising function | Matt Hyde | [https://stackoverflow.com/a/48226843](https://stackoverflow.com/a/48226843) |
| `randomColour` | `utils.ts` | Generates random colour HexCode       | Dan S     | [https://stackoverflow.com/a/5092846](https://stackoverflow.com/a/5092846)   |

### Libraries & Tools Used

| Libary     | Link                                                             |
|------------|------------------------------------------------------------------|
| NVM        | [https://github.com/nvm-sh/nvm](https://github.com/nvm-sh/nvm)   |
| Node       | [https://nodejs.org/en](https://nodejs.org/en)                   |
| Vite       | [https://vite.dev](https://vite.dev)                             |
| Yarn       | [https://yarnpkg.com](https://yarnpkg.com)                       |
| TypeScript | [https://www.typescriptlang.org](https://www.typescriptlang.org) |
| tabulator  | [https://tabulator.info](https://tabulator.info)                 |
| Svelte     | [https://svelte.dev](https://svelte.dev)                         |
| RsDB       | [https://rxdb.info](https://rxdb.info)                           |
| Vitest     | [https://vitest.dev](https://vitest.dev)                         |
